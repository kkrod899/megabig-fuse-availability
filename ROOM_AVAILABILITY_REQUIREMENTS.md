...
| 閲覧方式 | 公開範囲 | UI | 採用条件 |
|---|---|---|---|
| GitHub Pages | 原則インターネット公開 | SP週間一覧、今日の空き時間帯、鮮度表示、公式予約ボタン | 初期実装 |
...
| 対象日 | 必須 | 今日を含む向こう7日間。画面上の個別日フィルターは設けない |
| 利用開始時刻 | 必須 | 10:00以降 |
| 利用終了時刻 | 必須 | 17:00まで。2時間利用では開始候補を15:00までに制限する |
...
3. 各日について、予約画面が提示する開始時刻を10:00、10:15、10:30…15:00のように順番に設定する。
...
GitHub-hosted runnerの利用時間と予約サイトへの負荷を抑えるため、高頻度の全件走査は行わない。Asia/Tokyo基準で毎朝9時に1回実行する。

| `scan-and-deploy.yml`の起動 | 対象 | 初期頻度 | 目的 |
|---|---|---:|---|
| daily schedule | 今日～6日後 | 毎朝9時 | 10:00開始～17:00終了の週間表全体を走査し、Pagesを更新 |
| `workflow_dispatch` | 今日～6日後 | 手動 | 保守・動作確認時に7日間を再走査する |
...
| 8時間 | 32 | 約96秒＋画面読込 |
| 12時間 | 48 | 約144秒＋画面読込 |
| 7日間・10:00開始～17:00終了・2時間利用 | 147 | 約7～10分＋日付変更・画面読込 |
| 7日間・全営業時間 | 予約画面の候補数による | 初回実測で約24分以上となったため通常運用では使用しない |
...
| Pages上へGitHub tokenを置けない | 新規走査は毎朝9時のActionsに限定し、UIの再読込は保存済みJSONの取得だけにする |
...
```text
┌────────────────────────────────────────────────────────────┐
│ メガビッグ布施駅前店 フラットルーム          27分前に取得 │
│ 今日予約可能な時間帯: 10:15～17:00                       │
├────────────────────────────────────────────────────────────┤
│ 7/20（月）  ● 予約可能                         10:15～17:00│
│ 7/21（火）  ● 予約可能                         10:00～17:00│
│ 7/22（水）  × 空き時間なし                              — │
│ 7/23（木）  … 確認待ち                                  — │
├────────────────────────────────────────────────────────────┤
│                  [公式予約画面で確認する]                   │
...
週間ビューは開閉操作のない日付行を縦に7件並べ、各日には次を表示する。

- 予約可否を表す状態
- 予約可能な場合は、状態の右側に連続した空き時間帯
- 情報の鮮度は画面右上に1か所だけ表示

明細の開閉、個別確認時刻、監視条件、手動再読込ボタンは表示しない。画面下部には公式予約画面への大きなボタンを1つ表示する。監視条件の変更は`config/watch.json`をGitHub上で編集し、`Run workflow`を押して反映する。
...
30分利用を15分刻みで走査し、重なり合う予約可能結果を連続した時間帯へ結合する。

例: `10:00～10:30`、`10:15～10:45`、`10:30～11:00`が予約可能な場合は、次のように表示する。

```text
10:00～11:00
```

途中に予約不可の範囲がある場合は、`10:00～12:30 / 14:00～17:00`のように分けて表示する。これは30分利用の照会結果をまとめた目安であり、長時間利用が同一条件で予約できることを保証しない。希望時間を決めた後、公式予約画面で最終確認する。
...
| 初期表示 | 店舗・ルーム、今日予約可能な時間帯、今日から7日分を表示 |
| 日付行 | 開閉操作なしで、状態の右側に予約可能な時間帯を常時表示 |
...
| 更新方針 | 毎朝9時の週間結果を表示し、予約前の最終確認は公式予約画面で行う |
...
| SP UI | HTML/CSS/JavaScript + GitHub Pages | 320px以上で7日分の状態と時間帯を1画面で確認できる |
...
| 17 | 週間一覧を確認 | 操作せず各日の状態と予約可能時間帯を確認できる |
...
| 走査範囲 | 10:00～17:00。30分利用の開始候補は10:00～16:30 |
| 枠間隔 | 2～5秒 |
| 自動更新 | 向こう7日間を毎朝9時（JST）に1回 |
