...
| 項目 | 状態 |
|---|---|
| 320px以上のSP週間UI（各日1行・開閉操作なし） | 実装済み |
| 30分利用の結果を連結した空き時間帯表示 | 実装済み |
| `latest.json`のcacheなし再読込 | 実装済み |
| 60秒ごとの画面内自動再読込 | 実装済み |
| GitHub Pages配備workflow | 実装済み |
| GitHub Actions上の実予約画面走査 | 実装・動作確認済み |
| 毎朝9時の7日走査（10:00〜17:00、30分利用） | workflow実装済み |
...
毎朝9時（JST）に、30分利用を15分刻みで向こう7日分走査します。重なり合う予約可能結果は`10:15〜17:00`のような時間帯にまとめて表示します。ページでは保存済みの週間結果を即時表示し、目星を付けた後は公式予約画面で希望する利用時間を指定して最新状況を確認します。
